...
export const FACE_SHAPES = ['round', 'oval', 'square', 'heart', 'diamond'] as const
...
export function buildRecommendationPrompt(inputData: InputData): string {
  const d = (inputData?.data || {}) as Record<string, string>
  const isPhoto = inputData?.type === 'photo'
  ...
  const styleList = APPROVED_STYLES.map((s) => `- ${s.name} -> ${s.slug}`).join('\n')
...
export function buildEditPrompt(rec: Recommendation): string {
  return [
...
export function buildStockPrompt(rec: Recommendation): string {
  return [
...
function pickArray(parsed: unknown): unknown[] | null {
  ...
  if (Array.isArray(p.styles)) return p.styles
...
export function parseModelResponse(text: string | null | undefined): unknown {
  if (!text || typeof text !== 'string') return null
  let parsed = tryJsonParse(text)
  if (parsed) return parsed
  const stripped = text.replace(/^\s*```(?:json|JSON)?\s*\n?/g, '').replace(/\n?\s*```\s*$/g, '').trim()
  if (stripped !== text) {
    parsed = tryJsonParse(stripped)
    if (parsed) return parsed
  }
  const objMatch = text.match(/\{[\s\S]*\}/)
  if (objMatch) { parsed = tryJsonParse(objMatch[0]); if (parsed) return parsed }
  const arrMatch = text.match(/\[[\s\S]*\]/)
  if (arrMatch) { parsed = tryJsonParse(arrMatch[0]); if (parsed) return parsed }
  return null
}

export function parseRecommendations(text: string | null | undefined): unknown[] | null {
  return pickArray(parseModelResponse(text))
}

export function parseGenderCheck(parsed: unknown): string | null {
  if (!parsed || typeof parsed !== 'object') return null
  const value = (parsed as Record<string, unknown>).gender_check ?? (parsed as Record<string, unknown>).genderCheck
...
export const RECOMMENDATION_RESPONSE_SCHEMA = {
  type: 'OBJECT',
  properties: {
    gender_check: { type: 'STRING', enum: ['male', 'female', 'unclear', 'not_applicable'] },
    recommendations: {
      type: 'ARRAY',
      items: {
        type: 'OBJECT',
...
export function normalizeRecommendations(recs: unknown[]): Recommendation[] {
  if (!Array.isArray(recs)) return []
  return recs.slice(0, 3).map((r: any) => {
...
export function getFallbackRecommendations(data: Record<string, string> = {}): Recommendation[] {
  const face = (FACE_SHAPES as readonly string[]).includes(data.faceShape) ? data.faceShape : 'oval'
  const hair = (HAIR_TYPES as readonly string[]).includes(data.hairType) ? data.hairType : 'straight'
  ...
  return [
    {
      style_name: 'Low Taper Fade',
      match_score: 92,
      why_it_works: `A clean low taper fade balances a ${face} face shape and keeps ${hair} hair on top easy to style.`,
      barber_instructions:
    ...
    },
    {
      ...
      match_score: 88,
      why_it_works: `A mid taper draws focus upward and adds dimension that flatters a ${face} face with ${hair} hair.`,
      barber_instructions:
    ...
    },
    {
      ...
      match_score: 84,
      why_it_works: `A blowout taper adds height and a sharp silhouette that suits ${face} features and works well with ${hair} hair.`,
      barber_instructions:
...
export function extractInlineImage(data: any): string | null {
